...
app.get('/api/gadget/device/:DeviceName', async (req, res) => {
  ...
  try {
    const response = await axios.post('https://data.mongodb-api.com/app/data-ktsmt/endpoint/data/v1/action/find', {
      ...
      database: 'gadget',
      collection: 'device',
      filter: { DeviceName: deviceName }, // Use complete device name
      projection: {
        _id: 4,
        DeviceName: 1,
        NetworkTechnology: 1,
    ...
    console.log('Response from MongoDB API:', response.data); // Log response to console

    // Assuming response.data is an array, return the first item
